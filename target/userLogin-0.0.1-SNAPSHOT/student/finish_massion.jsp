...
<div class="container">
    ...
    <div class="main">
        <form action="/student/Submit_fileServlet" method="post" enctype="multipart/form-data">
            <div style="width: 80%;margin: 0 auto;overflow: hidden;margin-left: 9%;" id="div3" class="main">
                ...
                <div >
                    <br>
                    <p>${studentTask.getTask_content()}</p>
                    <%--任务附件部分--%>
                    <c:if test="${not empty studentTask.getTask_file()}">
                        <c:forEach items="${studentTask.getTask_file()}" var="fl" varStatus="vs">
                    ...
                    任务附件：<input id="file" name="task_file"  multiple="multiple" type="file" style="margin-top: 20px"><br>
                    <div >

                        <textarea onclick="show()" style="border: 1px solid black" id="task_content"name="task_content"   cols="70" rows="10">

                        </textarea><br>
                        <input type="submit" value="提交" style="width: 40px; height: 20px;margin-left: 400px;margin-top: 10px" >
                    </div>
                </div>
            </div>
            <script type="text/javascript">
